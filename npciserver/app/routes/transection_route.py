...
async def get_payment_methods(
    ...
    #     ]
    # }
async def send_to_kyc( server_url: str):
    import requests
    try:
        # you can also pass timeout=(connect, read)
        r = requests.get(server_url, timeout=5)
        print("KYC response status:", r.status_code)
    except Exception as e:
        print("KYC callback failed:", e)
...
async def process_transaction(
    ...
    if payer_vpa == payee_vpa:
        raise HTTPException(status_code=400, detail="Invalid transections")
    txn = Transaction(
        transaction_id= transaction_id,
        payer_uupi=sender.uupi,
        payee_uupi=receiver.uupi,
